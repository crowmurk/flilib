...
class FilteredTableMixin:
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        filter = self.filterset_class(
            self.request.GET,
            queryset=self.table_class._meta.model.objects.filter(
                **{self.related_name: self.object},
            ),
        )
        ...
        context_data.update({'filter': filter, self.context_table_name: table, })
        return context_data
...
class ActionTableDeleteMixin:
    ...
    каждой таблице задать словарь содержащий имя модели (
    action_table_model) и имя соответствующей кнопки (
    action_table_button).
    """
